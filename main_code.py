...
class DataBase:
    ...
    def table_creation_database(self):
        mydb = mysql.connector.connect(host="localhost", user="root", password="", database="mydatabase")

        mycursor = mydb.cursor(buffered=True)

        mycursor.execute("CREATE TABLE participants (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), password VARCHAR(255), contact VARCHAR(255))")
        mycursor.execute("CREATE TABLE records (id INT AUTO_INCREMENT PRIMARY KEY, time VARCHAR(255), total VARCHAR(255), accuracy VARCHAR(255))")
        mycursor.close()
        mydb.close()
        
    def insertion_record(self):
        mydb1 = mysql.connector.connect(host="localhost", user="root", password="", database="mydatabase")

        mycursor = mydb1.cursor(buffered=True)

        sql = "INSERT INTO participants (name, password, contact) VALUES (%s, %s,%s)"
        val = ("John", "Highway 21")
...
class Register:
    ...
    def Initial_Page(self):
        ...
        button1 = tk.Button(frame1, text="Register Yourself", command=self.register_window, font="Times 15",
                            cursor="hand2",
                            bg="black", fg="white").place(x=50, y=400, width=180)
        button2 = tk.Button(frame1, text="Proceed as Guest", command=self.proceed, font="Times 15", cursor="hand2",
                            bg="black",
    ...
    def register_window(self):
        ...
        DataBase.table_creation_database(self)
    ...
    def signup(self):
        ...
        button6 = tk.Button(frame1, text="Registration Window", command=self.register_data, font="Times 15",
    ...
    def signin(self):
        ...
        if (self.txt_uname.get() == "" or self.txt_pwd.get() == "" or self.txt_cont.get() == ""):
            ...
            if ((self.txt_uname.get()).format() == (self.txt_fname.get()).format() and (
                    self.txt_pwd.get()).format() == (self.txt_password.get()).format() and (
            self.txt_cont.get()).format() == (
    ...
    def register_data(self):
        ...
        sql = "INSERT INTO participants (name, password, contact) VALUES (%s, %s,%s)"
        val = (self.txt_fname.get(), self.txt_password.get(), self.txt_contact.get())
        mycursor.execute(sql, val)
